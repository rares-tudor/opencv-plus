...
canny_bgd_separator::canny_bgd_separator(const std::string& path) : path(path) {}

void canny_bgd_separator::subtract_bgd() 
{
	canny_edge_detection edgeDetector(path);

	cv::Mat original = cv::imread(path, cv::IMREAD_COLOR); // read the original image
